...
static std::vector<std::map<std::string, std::variant<int, std::string>>>
    symbol_tables;
//符号表，其中vector第一个元素为全局符号表，最后一个元素为当前所在块的符号表。
//这样当我们退出一个语句块时，只需要从 ```vector``` 中弹出一个元素；
//而进入一个语句块时，只需要向 ```vector``` 中加入一个元素。
...
public:
    ...
    {
        std::cout << "decl @getint(): i32" << std::endl;               //输出内置函数声明
        ...
        std::cout << "decl @stoptime()" << std::endl << std::endl;
                                                                        //更新函数表
        function_table["getint"] = "@getint";                           //函数名
        ...
        function_ret_type["getint"] = "int";                            //返回值
        ...
        function_param_num["getint"] = 0;                               //参数数量
        ...
        std::map<std::string, std::variant<int, std::string>> global_syms;//全局符号表
